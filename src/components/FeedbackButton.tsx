...
import { MessageSquarePlus, Loader2, Camera, X } from 'lucide-react';
...
export function FeedbackButton() {
  ...
  const captureScreenshot = async () => {
    setCapturing(true);
    try {
      // Briefly hide the dialog so we capture the underlying page
      setOpen(false);
      await new Promise(r => setTimeout(r, 250));
      const canvas = await html2canvas(document.body, {
        useCORS: true,
        scale: 0.6, // smaller / faster
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.error(`Could not send feedback: ${err?.message || err}`);
    } finally {
      setSubmitting(false);
    }
  };
